...
namespace AlpineCommerce\StoreSetup\Setup\Patch\Data;
...
class CreateStores implements DataPatchInterface
{
    ...
    private function createStore(
        ...
        if ($existing->getId()) {
            ...
            $this->assignTheme((int) $existing->getId(), $code);
            return;
        }
        ...
        $this->assignTheme((int) $store->getId(), $code);
    }
    ...
    private function assignTheme(int $storeId, string $storeCode): void
    {
        ...
        $themeId = $connection->fetchOne(
            $connection->select()->from($themeTable, ['theme_id'])->where('code = ?', 'AlpineCommerce/Training')
        );

        if (!$themeId) {
            $this->logger->info("Training DataPatch: Theme 'AlpineCommerce/Training' not found - skipping assignment for $storeCode");
            return;
        }
