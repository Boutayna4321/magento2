...
| S26 | Jumelles REST / SOAP / GraphQL (décision D4 = B) : un interrupteur par endpoint jumeau, **désactivé par défaut**, réglable par store view | `ConfigTest::testTwinBlockingDefaultsToOff` (valeur par défaut de chaque interrupteur dans `config.xml`), `testTwinBlockingIsReadPerStoreView` | U | **A** |
